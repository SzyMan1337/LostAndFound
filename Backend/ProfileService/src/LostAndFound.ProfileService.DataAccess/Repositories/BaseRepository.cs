...
namespace LostAndFound.ProfileService.DataAccess.Repositories
{
    public class BaseRepository<T> : IRepository<T> where T : IDocument
    {
        ...
        protected readonly IMongoProfileServiceDbContext _context;

        public BaseRepository(IMongoProfileServiceDbContext profileServiceDbContext)
        {
            _context = profileServiceDbContext ?? throw new ArgumentNullException(nameof(profileServiceDbContext));
            _collection = profileServiceDbContext.GetCollection<T>(GetCollectionName());
        }
