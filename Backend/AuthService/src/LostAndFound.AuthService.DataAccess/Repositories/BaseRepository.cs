...
namespace LostAndFound.AuthService.DataAccess.Repositories
{
    public class BaseRepository<T> : IRepository<T> where T : IDocument
    {
        ...
        protected readonly IMongoAuthServiceDbContext _context;

        public BaseRepository(IMongoAuthServiceDbContext authServiceDbContext)
        {
            _context = authServiceDbContext ?? throw new ArgumentNullException(nameof(authServiceDbContext));
            _collection = authServiceDbContext.GetCollection<T>(GetCollectionName());
        }
